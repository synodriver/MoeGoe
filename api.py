...
class Cleaner:
    def __init__(self, configfile: str):
        self.cleanernames = get_hparams_from_file(
            str(Path(__file__).parent / configfile)
        ).data.text_cleaners

    def main(self, req: func.HttpRequest) -> func.HttpResponse:
        text = req.params.get("text")
        if not text:
            return func.HttpResponse("400 BAD REQUEST: null text", status_code=400)
        try:
            return func.HttpResponse(
                _clean_text(unquote(text), self.cleanernames), status_code=200
            )
        except:
            return func.HttpResponse("400 BAD REQUEST: invalid text", status_code=400)
...
class Speaker:
    ...
    def get_text(self, text: str, cleaned=False):
        if cleaned:
            ...
            text_norm = text_to_sequence(
                text, self.hps_ms.symbols, self.hps_ms.data.text_cleaners
            )
    ...
    def main(self, req: func.HttpRequest) -> func.HttpResponse:
        ...
        if not speaker_id:
            return func.HttpResponse(
                "400 BAD REQUEST: null speaker id", status_code=400
            )
        try:
            speaker_id = int(speaker_id)
        except:
            return func.HttpResponse(
                "400 BAD REQUEST: invalid speaker id", status_code=400
            )
        if speaker_id not in range(self.hps_ms.data.n_speakers):
            return func.HttpResponse(
                "400 BAD REQUEST: speaker id out of range", status_code=400
            )
        format = req.params.get("format")
        if not format:
            format = "ogg"
        if format not in ("ogg", "mp3", "wav"):
            return func.HttpResponse("400 BAD REQUEST: invalid format", status_code=400)
        try:
